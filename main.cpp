...

{
	for (int i = 0; i < dev_vec.size(); i++)
	{
		...
		aud_strct[i].file.open(file_str[0] + "/audio_" + to_string(i) + "_16bit_" + to_string(ar) + "Hz_" + to_string(ac) + "ch_" + file_str[1] + ".wav", ios::binary);
...
int main(int argc, char* argv[])
{
	...
	if (REC_INIT)
	{
		...
		while (true)
		{
			...
			loop_break_time = static_cast<double>(main_thread_timestamp) + read_duration_ms - (frame_duration + 1);
			...
			actual_read_time_ms = msTimeStamp(startTime);
